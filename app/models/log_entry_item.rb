class LogEntryItem < ActiveRecord::Base
  ...
  def set_values
    ...
    self.value_datetime = StringLib.cast(self.value, :datetime) if StringLib.is_datetime?(self.value)
    self.value_date = StringLib.cast(self.value, :date) if StringLib.is_date?(self.value)
    self.value_time = StringLib.cast(self.value, :time) if StringLib.is_time?(self.value)
